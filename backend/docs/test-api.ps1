...
#  Version: 0.10.0  |  Updated: 2026-07-04
#
#  Controllers tested:
...
function AssertAny($Label, $Actual, $ExpectedValues) {
    if ($ExpectedValues -contains $Actual) {
        Write-Host "  [PASS] $Label" -ForegroundColor Green
        $script:Pass++
    }
    else {
        Write-Host "  [FAIL] $Label -- expected one of [$($ExpectedValues -join ', ')], got $Actual" -ForegroundColor Red
        $script:Fail++
    }
}
...
# Ensure seeded entities are active before read/write checks.
Write-Host "`n=== STEP 1.5: Setup baseline state ===" -ForegroundColor Cyan
AssertAny "Setup product unarchive returns 204 or 404" `
(Call "PATCH" "$BaseUrl/api/products/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/unarchive" $OfficerToken) @(204, 404)
AssertAny "Setup document unarchive returns 204 or 404" `
(Call "PATCH" "$BaseUrl/api/documents/cccccccc-cccc-cccc-cccc-cccccccccccc/unarchive" $OfficerToken) @(204, 404)
AssertAny "Setup submission package unarchive returns 204 or 404" `
(Call "PATCH" "$BaseUrl/api/submissionpackages/eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee/unarchive" $OfficerToken) @(204, 404)
...
$updDoc = @{
    title = "CoA v1.1"; type = "CertificateOfAnalysis"; status = "Final"
...
# ============================================================
Write-Host "`n=== STEP 9: Cleanup — unarchive seeded entries ===" -ForegroundColor Cyan
# ============================================================

Assert "PATCH /api/products/{id}/unarchive -- viewer 403" `
(Call "PATCH" "$BaseUrl/api/products/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/unarchive" $ViewerToken) 403
Assert "PATCH /api/products/{id}/unarchive -- officer 204" `
(Call "PATCH" "$BaseUrl/api/products/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/unarchive" $OfficerToken) 204

Assert "PATCH /api/documents/{id}/unarchive -- viewer 403" `
(Call "PATCH" "$BaseUrl/api/documents/cccccccc-cccc-cccc-cccc-cccccccccccc/unarchive" $ViewerToken) 403
Assert "PATCH /api/documents/{id}/unarchive -- officer 204" `
(Call "PATCH" "$BaseUrl/api/documents/cccccccc-cccc-cccc-cccc-cccccccccccc/unarchive" $OfficerToken) 204

Assert "PATCH /api/submissionpackages/{id}/unarchive -- viewer 403" `
(Call "PATCH" "$BaseUrl/api/submissionpackages/eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee/unarchive" $ViewerToken) 403
Assert "PATCH /api/submissionpackages/{id}/unarchive -- officer 204" `
(Call "PATCH" "$BaseUrl/api/submissionpackages/eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee/unarchive" $OfficerToken) 204
